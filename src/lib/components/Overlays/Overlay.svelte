...
{#if $overlayStore && $overlayStore.id === id && type === "modal"}
	<div
		{id}
		transition:slide={{ duration: 250, axis: "x" }}
		class="translate-x-0 open fixed top-0 end-0 left-0 right-0 transition-all duration-300 transform flex flex-col z-[80] bg-white border-s dark:bg-gray-800 dark:border-gray-700"
		tabindex="-1"
	>
		<div
			class="flex justify-between items-center py-3 px-4 border-b dark:border-gray-700"
